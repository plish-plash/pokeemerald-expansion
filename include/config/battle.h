...
#define B_X_ITEMS_CROSSUSE          FALSE      // In Gen3, you can only use X item on the current battler, but in Gen7 you can use an X item on any friendly battler in double battles (we are not sure in which gen the change occured)
...
#define B_CRITICAL_CAPTURE_IF_OWNED     GEN_3      // In Gen9, a capture appear critical if the Pokémon you are trying to catch already has a dex entry (has already been caught)
