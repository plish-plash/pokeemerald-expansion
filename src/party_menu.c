...
static bool8 IsValidActionForQuickSwap(u8 partyMenuType)
{
    return (partyMenuType == PARTY_ACTION_CHOOSE_MON
         || partyMenuType == PARTY_ACTION_SWITCH
         || partyMenuType == PARTY_ACTION_SWITCHING
         || partyMenuType == PARTY_ACTION_CHOOSE_FAINTED_MON);
}
...
static u16 PartyMenuButtonHandler(s8 *slotPtr)
{
    ...
    if (JOY_NEW(SELECT_BUTTON) && CalculatePlayerPartyCount() >= 2 && IsValidActionForQuickSwap(gPartyMenu.action))
    {
