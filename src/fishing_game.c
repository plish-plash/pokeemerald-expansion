...
static const u8 gText_CaughtPokemonMagnet[] = _("The magnet pulled in a POKéMON!{PAUSE_UNTIL_PRESS}");
    ...
    switch (TaskState)
    {
        ...
        case FISHTASK_OBTAIN_ITEM:
            ...
            if (!IsTextPrinterActiveOnWindow(0) && (JOY_NEW(A_BUTTON) || JOY_NEW(B_BUTTON)))
            {
                ...
                {
                    PlaySE(SE_SELECT);
                    StringCopy(gStringVar3, gPocketNamesStringsTable[GetItemPocket(gSpecialVar_ItemId)]);
                }
